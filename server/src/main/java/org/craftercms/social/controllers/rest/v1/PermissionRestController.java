...
public class PermissionRestController {
    ...
    @ModelAttribute
    public boolean isAllowed(@PathVariable String ugcId, @PathVariable String action) {
        log.debug(String.format("Is allowed id=%s action=%s", ugcId, action));
        try {
            return permissionService.allowed(ugcId, action, getProfile().getId(), getProfileRoles());
        } catch (Exception e) {
            return false;
        }
    }
    ...
    private org.craftercms.security.api.UserProfile getProfile() {
        return RequestContext.getCurrent().getAuthenticationToken().getProfile();
    }

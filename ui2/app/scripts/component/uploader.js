...
(function (S) {
	...
	var $ = S.$,
		compile = S.TemplateEngine.compile;
	...
	$.widget('blueimp.fileupload', $.blueimp.fileupload, {
		...
		options: {
					...
					data.context.each(function (index) {
						...
						that._transition($(this)).done(function () {
							...
							try {
								...
								renderData = [$.extend({}, file, response)];
							} catch (ex) {
								renderData = [file];
							}

							template = that._renderDownload(renderData).replaceAll(node);
					...
					data.context.each(function (index) {
						if (data.errorThrown !== 'abort') {
							...
							try {
								...
								file.error = response.localizedMessage || response.message;
							} catch (ex) {
			...
			processstop: function (e) {
				...
				if (isMSIE() && MSIEversionNumber() < 10) {
					var $el = $(this);
				...
				function isMSIE() {
					...
					(msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) ? true : false;
				}
		...
		_initTemplates: function () {
			...
			options.templatesContainer =
				this.document[0].createElement(options.filesContainer.prop('nodeName'));
			if (options.uploadTemplateId) {
				options.uploadTemplate = compile(
					document.getElementById(options.uploadTemplateId).innerHTML);
			}
			if (options.downloadTemplateId) {
				options.downloadTemplate = compile(
					document.getElementById(options.downloadTemplateId).innerHTML);
			}
		},
		...
		_initFilesContainer: function () {
			...
			if (options.filesContainer === undefined) {
				options.filesContainer = this.element.find('.cs-files-list');
		...
		_initSpecialOptions: function () {
			...
			if (!(function () {
				...
				this.__dragenter = function () {
					$el.addClass('dropbox-dragover');
				};
				this.__dragleave = function () {
					$el.removeClass('dropbox-dragover');
				};
